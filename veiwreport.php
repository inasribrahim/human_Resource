...
<article>
		...
		<?php 
			...
			if ( $count > 0) { 
				echo "<table id=".'view-report'." cellpadding="."10px"." border-style ><tr><th>ID</th><th>Name</th><th>Code</th> <th>Mobile</th> <th>Department</th><th>Profile</th><th>Salary</th>";

			while($row = $result->fetch_assoc()) {
			echo "<tr><td>".$row["userId"]."</td><td>".$row["fName"]." ".$row["lName"]."</td><td>".$row["codeEmp"]."</td><td>".$row["mobile"]."</td><td>".$row["department"]."</td><td>".$row["facePrint"]."</td><td>".$row["grossSalary"]."</td></tr>"; }
			echo "</table>";
			echo"<br>";
			 }
			else { echo "0 results"; }
